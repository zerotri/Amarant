...
#include <machine/x86/kheap.h>
#include <machine/x86/paging.h>
...
static int32_t find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    HeapArray_t::index_t iterator = 0;
    while (iterator < heap->index->size())
    {
        header_t *header = heap->index->lookup(iterator);
...
heap_t *create_heap(uint32_t start, uint32_t end_addr, uint32_t max, uint8_t supervisor, uint8_t readonly)
{
    ...
    heap->index = new HeapArray_t( (void*)start, HEAP_INDEX_SIZE);
...
void free(void *p, heap_t *heap)
{
        ...
        test_header->is_hole)
    {
        ...
        int32_t iterator = 0;
        while ( (iterator < heap->index->size()) &&
                (heap->index->lookup(iterator) != (void*)test_header) )
            iterator++;
    ...
    if ( (uint32_t)footer+sizeof(footer_t) == heap->end_address)
    {
        ...
        else
        {
            ...
            HeapArray_t::index_t iterator = 0;
            while ( (iterator < heap->index->size()) &&
                    (heap->index->lookup(iterator) != test_header) )
                iterator++;

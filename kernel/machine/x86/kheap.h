...
//#include "common.h"
#include <machine/x86/ordered_array.h>
#include <stdint.h>
...
struct header_t_less
{
  static bool compare (const header_t* x, const header_t* y)
	{
		return ((x->size) < (y->size)) ? true : false;
	};
};
typedef OrderedArray<header_t*, int32_t, header_t_less > HeapArray_t;
typedef struct
{
    HeapArray_t* index;
